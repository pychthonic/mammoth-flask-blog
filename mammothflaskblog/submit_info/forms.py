...
class VolunteerForm(FlaskForm):
	...
	volunteer_email = StringField(
			'Email',
			validators=[DataRequired(), Email(), Length(max=50)])
	street_addy = StringField(
			'Street Address',
			validators=[Length(max=50)])
	apt_suite_bldg = StringField(
			'Apt / Suite / Bldg',
			validators=[Length(max=20)])
	volunteer_city = StringField('City', validators=[Length(max=50)])
	volunteer_state = StringField(
			'State / Province / Region',
			validators=[Length(max=50)])
	volunteer_zip = StringField(
			'Postal / Zip Code',
			validators=[Length(max=20)])
	volunteer_country = StringField(
			'Country',
			validators=[Length(max=20)])
	...
	other_volunteering_deets = TextField(
			"Other ways I can help that aren't listed above:",
...
class MemberForm(FlaskForm):
	...
	apt_suite_bldg = StringField(
			'Apt / Suite / Bldg',
			validators=[Length(max=20)])
	member_city = StringField(
			'City',
			validators=[Length(max=50)])
	member_state = StringField(
			'State / Province / Region',
			validators=[Length(max=50)])
	member_zip = StringField(
			'Postal / Zip Code',
			validators=[Length(max=20)])
	member_country = StringField(
			'Country',
			validators=[Length(max=20)])
	submit = SubmitField('Submit')
...
class EmailListForm(FlaskForm):
	email_address_to_add = StringField(
			'Email',
			validators=[DataRequired(), Email(), Length(max=50)])
	submit = SubmitField('Submit')