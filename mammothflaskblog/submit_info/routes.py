from flask import (render_template, url_for, flash,
				   redirect, Blueprint)
from mammothflaskblog.submit_info.forms import VolunteerForm, MemberForm, EmailListForm
...
def sign_up_to_volunteer():
	...
	if form.validate_on_submit():
		...
		msg = Message('New volunteer form submitted from website!', sender=Config.MFB_EMAIL,
...
Can help with:<br>"""
	...
	return render_template('volunteer.html', title='Sign Up To Help!', form=form,
						   legend='Sign Up To Help!', carousel_slides1=carousel_slides1, carousel_slides2=carousel_slides2)
...
def submit_info_main():
	...
	carousel_slides1 = enumerate(carousel_slides)
	carousel_slides2 = enumerate(carousel_slides)
	return render_template('submit_info.html', title="Take Action", carousel_slides1=carousel_slides1, carousel_slides2=carousel_slides2)
...
def sign_up_for_membership():
	...
	if form.validate_on_submit():
		...
		msg = Message('New member form submitted from website!', sender=Config.MFB_EMAIL,
					  recipients=[Config.MFB_EMAIL])
		message_string = f"""<h4>Name: {form.first_name.data} {form.last_name.data}<br>
Phone: {form.phone_number.data}<br>
...
Country: {form.member_country.data}<br><br>"""
		...
		flash('Thanks for signing up! We will be in touch soon :)', 'success')
		return redirect(url_for('main.home'))
	return render_template('become-a-member.html', title='Become a Member!', form=form,
						   legend='Become a Member!', carousel_slides1=carousel_slides1, carousel_slides2=carousel_slides2)
...
def sign_up_for_email_list():
	...
	if form.validate_on_submit():
		...
		msg = Message('New email address submitted for Email Announcements!', sender=Config.MFB_EMAIL,
					  recipients=[Config.MFB_EMAIL])
		message_string = f"""<h4>
...
<br><br>"""
		...
		flash('Thanks! We will add your email to our Email Announcements List :)', 'success')
		return redirect(url_for('main.home'))
	return render_template('email_list_sign_up.html', title='Email Announcements List Sign-Up', form=form,
						   legend='Email Announcements List Sign-Up', carousel_slides1=carousel_slides1, carousel_slides2=carousel_slides2)