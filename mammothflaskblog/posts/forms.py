			...
			if not isinstance(data, str):
				...
				if isinstance(self.upload_set, Iterable):
					...
					if filename.split('.')[1] not in self.upload_set:
						flash(f"File does not have approved extension: {', '.join(self.upload_set)}", 'success')
...
class PostForm(FlaskForm):
	...
	main_photo = FileField('Main Photo', validators=[FileAllowed(['jpg', 'jpeg', 'png'], 'File must be .jpg, .jpeg or .png. Consult webmaster if you have a question about converting.')])
	main_photo_caption = StringField('Main Photo Caption', validators=[Length(max=150)])
	extra_photos = MultipleFileField('Additional Photos', default=None, validators=[MultiFileAllowed(['jpg', 'jpeg', 'png'])])
	
	extra_photos_captions = FieldList(StringField('Additional Photo Caption', default="No caption", validators=[Length(max=150)]))
	link_href = StringField('If adding a relevant link which will fall below the Content section, copy complete HTTP URL here ( example: https://www.abolitionistlawcenter.org ):',
