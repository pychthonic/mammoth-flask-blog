...
from mammothflaskblog.models import Post, CarouselSlider
...
def new_post():
	...
	if form.validate_on_submit():
		...
		if form.main_photo.data:
			...
			filename = "image" + str(post.id) + os.path.splitext(main_image.filename)[1]
			path_plus_filename = os.path.join('mammothflaskblog/static/images', filename)
			main_image.save(path_plus_filename)
			main_image_filename_for_db = os.path.join('images', filename)
			post.main_image_filename = main_image_filename_for_db
			db.session.commit()

		if form.extra_photos.data != ['']:				# THIS ALWAYS PASSES AS TRUE FOR SOME REASON
			post = db.session.query(Post).order_by(Post.id.desc()).first()
			...
			for image in extra_images:
				filename = "image" + str(post.id) + letters[extra_image_count] + os.path.splitext(image.filename)[1]
				path_plus_filename = os.path.join('mammothflaskblog/static/images', filename)
				image.save(path_plus_filename)
				if extra_image_count > 0:
					extra_image_list_string += '|' + os.path.join('images', filename)
				else:
					extra_image_list_string += os.path.join('images', filename)
				extra_image_count += 1
...
def post(post_id):
	...
	return render_template('post.html', 
							...
							facebook_share_link = facebook_share_link, carousel_slides1=carousel_slides1, carousel_slides2=carousel_slides2)
...
def update_post(post_id):
	...
	if form.validate_on_submit():
		...
		if form.main_photo.data:
			if post.main_image_filename:
				file_path_to_delete = "mammothflaskblog/static/" + post.main_image_filename
		...
		extra_files_submitted = False	# this will be changed to true if extra files submitted
		if form.extra_photos.data != ['']:
			if post.extra_images_filenames:
				...
				for file_to_delete in filenames_to_delete:
					...
					file_path_to_delete = "mammothflaskblog/static/" + file_to_delete
					if os.path.isfile(file_path_to_delete):
						os.remove(file_path_to_delete)
		...
		if form.extra_photos_captions.entries and not extra_files_submitted:
			...
			for count in range(len(extra_images)):
				image_caption_dict[count] = [extra_images[count], captions[count]]
			...
			for count,image_caption_entry in image_caption_dict.items():
				filename = "image" + str(post.id) + letters[count] + os.path.splitext(image_caption_entry[0])[1]
				if extra_image_count > 0:
					extra_image_list_string += '|' + os.path.join('images', filename) + "~" + image_caption_entry[1]
				else:
					extra_image_list_string += os.path.join('images', filename) + "~" + image_caption_entry[1]
				extra_image_count += 1
				if extra_image_count == 10:
	...
	return render_template('create_post.html', title='Update Post',
						   form=form, post=post, image_filename_caption_dict=image_filename_caption_dict, legend='Update Post')
	...
	if post.extra_images_filenames:
		...
		for count in range(len(filenames_to_delete)):
			if "~" in filenames_to_delete[count]:
				filenames_to_delete[count] = filenames_to_delete[count].split("~")[0]
